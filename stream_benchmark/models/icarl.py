...
from stream_benchmark.utils.buffer import Buffer, icarl_replay
from stream_benchmark.models.__base_model import BaseModel
import numpy as np
from stream_benchmark.utils.batch_norm import bn_track_stats


def fill_buffer(self, mem_buffer: Buffer, train_loader, t_idx: int) -> None:
        """
        Adds examples from the current task to the memory buffer
        by means of the herding strategy.
        :param mem_buffer: the memory buffer
        :param train_loader: the train dataloader from which take the examples
        ...
        for x, y in loader:
            ...
            x, y, not_norm_x = x[mask], y[mask], x.clone()[mask]
            if not x.size(0):
                continue
...
class ICarl(BaseModel):
    name = 'icarl'
    description = 'Continual Learning via iCaRL.'
    link = "https://arxiv.org/abs/1611.07725"

    def __init__(self, backbone, loss, lr, buffer_size, batch_size, minibatch_size, task_start_idx, **_):
        super(ICarl, self).__init__(backbone, loss, lr)
        # Instantiate buffers
        self.buffer = Buffer(buffer_size, self.device)
        self.task_start_idx = task_start_idx
        self.eye = torch.eye(self.task_start_idx[-1]).to(self.device)
    ...
    def begin_task(self, train_loader, task_start_idx):
        return icarl_replay(self, train_loader, task_start_idx)

    def end_task(self, train_loder, *_) -> None:
        self.old_net = deepcopy(self.net.eval())
        self.net.train()
        with torch.no_grad():
            fill_buffer(self, self.buffer, train_loder, self.task)
        self.task += 1
        self.class_means = None
    ...
    def compute_class_means(self) -> None:
        ...
        for _y in self.classes_so_far:
            ...
            with bn_track_stats(self, False):
                allt = None
                while len(x_buf):
                    batch = x_buf[:self.batch_size]
                    x_buf = x_buf[self.batch_size:]
