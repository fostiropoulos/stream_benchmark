...
class Gem(BaseModel):
    ...
    def __init__(self, backbone, loss, lr, buffer_size, minibatch_size, task_start_idx, gem, **_):
        super(Gem, self).__init__(backbone, loss, lr)
        self.current_task = 0
        self.buffer = Buffer(buffer_size, self.device)
        self.minibatch_size = minibatch_size
        self.buffer_size = buffer_size
        self.gamma = gem['gamma']
        self.n_tasks = len(task_start_idx)
    ...
    def end_task(self, train_loader, task_start_idx, *_):
        ...
        samples_per_task = self.buffer_size // self.n_tasks
        sampled = 0
        for i, data in enumerate(train_loader):
            inputs, labels = data
            labels = task_start_idx + labels
            if samples_per_task < sampled + inputs.shape[0]:
                rand_idx = torch.randperm(inputs.shape[0])[:samples_per_task - sampled]
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            buf_inputs, buf_labels, buf_task_labels = self.buffer.get_data(
                self.buffer_size, transform=None)
        ...
        if not self.buffer.is_empty():
            ...
            if (dot_prod < 0).sum() != 0:
                project2cone2(self.grads_da.unsqueeze(1),
                              torch.stack(self.grads_cs).T, margin=self.gamma)
                # copy gradients back
